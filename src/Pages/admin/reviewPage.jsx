...
import {
    FiFilter, FiRefreshCw, FiSearch, FiCalendar,
} from "react-icons/fi";
...
export default function AdminReviewPage() {
    ...
    // Rating chart & keyword freq for PDF
    ...
    const keywordFreq = useMemo(() => {
        const stop = new Set([
            "the","and","a","is","to","for","of","in","on","it","this","that","with",
            "at","as","was","but","be","are","we","our","your","you"
        ]);
        const map = {};
        reviews.forEach((r) => {
            (r.comment || "")
    ...
    const handleCreateReport = async () => {
        ...
        doc.setFontSize(14);
        doc.text("BuyNest Review Analysis Report", pageWidth/2, 30, {align:"center"});
        doc.setFontSize(10);
    ...
    const sendReplyEmail = async (review) => {
        try {
            await axios.post("http://localhost:5000/api/reviews/reply", {
                email: review.email,
                usersName: review.usersName,
            ...
            <div className="mb-4 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-2 text-slate-600">
                        <FiFilter /><span className="text-sm font-medium">Filters</span>
                    </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        {filtered.map((r, index) => (
                            <tr key={r.reviewId || r._id || index}
                                className={index % 2 === 0 ? "bg-white":"bg-slate-50"}>
                                <Td className="max-w-[220px] truncate text-emerald-700 font-medium">{r.reviewId}</Td>
                                <Td>{r.usersName || "-"}</Td>
                                ...
                                <Td className="max-w-[420px]"><span className="block truncate">{r.comment || "-"}</span></Td>
                                <Td className="text-center">
                                    <button
                                        onClick={()=>sendReplyEmail(r)}
                        ...
                        {filtered.length === 0 && (
                            <tr>
                                <td className="py-10 px-4 text-center text-slate-500 italic" colSpan={6}>
...
function RatingStars({ value = 0, outOf = 5 }) {
    ...
    const Star = ({ type }) => (
        <span className={type === "full" ? "text-amber-500" :
            type === "half" ? "text-amber-500" : "text-slate-300"}>
            {type === "half" ? "☆" : "★"}
        </span>
    );
    return (
...
function Th({ children, className = "" }) {
    return <th className={`py-3 px-4 text-xs font-semibold uppercase ${className}`}>{children}</th>;
}
