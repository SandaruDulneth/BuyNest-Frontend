  ...
  const derivedImage =
    ...
    (
      (Array.isArray(images) && images.length > 0
        ? (typeof images[0] === "string" ? images[0] : images[0]?.url || images[0]?.publicUrl)
        : null
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 pr-4">
        <h3 className="line-clamp-2 text-[15px] font-semibold text-gray-800">{name}</h3>
      ...
      <div className="flex-shrink-0 h-32 w-32">
        <img src={derivedImage} alt={name} className="h-full w-full object-contain" loading="lazy" />
      </div>
    </div>
  );
}
